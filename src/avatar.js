const options = {
  // isCircle: [true, false],
  circleColor: ['#e6194B', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#42d4f4', '#f032e6', '#fabed4', '#469990', '#dcbeff', '#9A6324', '#fffac8', '#800000', '#aaffc3', '#000075', '#a9a9a9', '#ffffff', '#000000'],
...
export async function avatarHash (str) {
  const hash = await crypto.subtle
    ...
    .then(h => new DataView(h))

  let offset = 0
  const params = { isCircle: true }
  for (const [key, values] of Object.entries(options)) {
    const len = values.length
    const i = hash.getUint8(offset++) % len
